...
CREATE TABLE IF NOT EXISTS `mydb`.`user` (
  ...
  `details` VARCHAR(45) NULL COMMENT '이메일\n닉네임\n이름\n비밀번호\n성별\n전공\n시\n구\n동\n상세주소',
  PRIMARY KEY (`email`));
...
CREATE TABLE IF NOT EXISTS `mydb`.`contents` (
  `no` INT NOT NULL AUTO_INCREMENT,
  `nickname` VARCHAR(45) NOT NULL COMMENT '공모전 등록(admin)\n프로젝트 등록한 사람',
...
CREATE TABLE IF NOT EXISTS `mydb`.`team` (
  ...
  PRIMARY KEY (`no`, `leader`),
  CONSTRAINT `fk_team_contents`
    FOREIGN KEY (`no` , `leader`)
    REFERENCES `mydb`.`contents` (`no` , `nickname`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'team 등록';
...
CREATE TABLE IF NOT EXISTS `mydb`.`member` (
  `no` INT NOT NULL,
  `leader` VARCHAR(45) NOT NULL COMMENT '팀장 이름',
  `nickname` VARCHAR(45) NOT NULL COMMENT '팀원의 nickname',
  `part` VARCHAR(45) NULL,
  PRIMARY KEY (`no`, `leader`, `nickname`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`applymember` (
  ...
  `leader` VARCHAR(45) NOT NULL COMMENT '지원한 팀의 리더',
  `nickname` VARCHAR(45) NOT NULL,
