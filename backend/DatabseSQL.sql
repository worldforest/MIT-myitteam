...
CREATE TABLE IF NOT EXISTS `mydb`.`team` (
  ...
  CONSTRAINT `fk_team_contents`
    ...
    REFERENCES `mydb`.`contents` (`no` , `email`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'team 등록';
...
CREATE TABLE IF NOT EXISTS `mydb`.`teaminfo` (
  `no` INT NOT NULL,
  `leader` VARCHAR(255) NOT NULL COMMENT '팀장',
